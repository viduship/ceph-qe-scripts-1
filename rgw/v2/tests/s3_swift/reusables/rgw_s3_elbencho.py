...
def get_rgw_services_by_realm(ssh_con=None, site_name="local", realm_name=None):
    ...
    log.info(f"  Running: {config_cmd} (will filter for realm '{realm_name}' in Python)")
...
def get_remote_endpoint_elbencho(zone_name=None, ssh_con=None):
    ...
    try:
        ...
        stdin, stdout, stderr = ssh_con.exec_command("sudo ceph orch host ls", get_pty=True)
...
def stop_rgw_sync_services(ssh_con=None, site_name="secondary"):
    ...
    try:
        ...
        if ssh_con:
            stdin, stdout, stderr = ssh_con.exec_command("sudo ceph orch ls", get_pty=True)
            orch_ls_output = stdout.read().decode()
        else:
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            for service_name in rgw_services:
                ...
                if ssh_con:
                    stdin, stdout, stderr = ssh_con.exec_command(f"sudo {stop_cmd}", get_pty=True)
                    output = stdout.read().decode().strip()
                else:
            ...
            if ssh_con:
                stdin, stdout, stderr = ssh_con.exec_command(f"sudo {verify_cmd}", get_pty=True)
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            for service_name in rgw_services:
                ...
                if ssh_con:
                    stdin, stdout, stderr = ssh_con.exec_command(f"sudo {start_cmd}", get_pty=True)
                    output = stdout.read().decode().strip()
                else:
